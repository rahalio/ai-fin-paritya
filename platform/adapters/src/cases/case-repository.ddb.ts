/**
 * CaseRepositoryDdb — in-memory sandbox implementation (handwritten over codegen stub).
 * Domain: cases
 */

import type { CaseRepository } from "@paritya/services/cases";
...
export class CaseRepositoryDdb implements CaseRepository {
  ...
  async listCases(input: Parameters<CaseRepository['listCases']>[0]): Promise<Awaited<ReturnType<CaseRepository['listCases']>>> {
    const table = memoryTable("cases:case-repository.ddb");
  ...
  async createCase(input: Parameters<CaseRepository['createCase']>[0]): Promise<Awaited<ReturnType<CaseRepository['createCase']>>> {
    ...
    const entity = {
      ...
      updatedAt: now,
      status: raw.status ?? (("createCase".includes("submit") || "createCase".includes("Submit")) ? "submitted" : raw.status ?? "draft"),
      immutable: raw.immutable ?? ("createCase".toLowerCase().includes("gate") ? true : raw.immutable),
      ...
      decidedAt: raw.decidedAt ?? ("createCase".toLowerCase().includes("gate") ? now : raw.decidedAt),
      decidedBy: raw.decidedBy ?? ("createCase".toLowerCase().includes("gate") ? "supervisor" : raw.decidedBy),
  ...
  async updateCase(input: Parameters<CaseRepository['updateCase']>[0]): Promise<Awaited<ReturnType<CaseRepository['updateCase']>>> {
    ...
    const entity = {
      ...
      id: id || existing.id,
      updatedAt: memoryNow(),
      status: "updateCase".toLowerCase().includes("revoke") ? "revoked" : (raw.status ?? existing.status),
    };
