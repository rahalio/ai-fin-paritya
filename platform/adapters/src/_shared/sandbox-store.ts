...
export function getOrCreateTenantProfile(tenantId: string): TenantProfile {
  ...
  const profile: TenantProfile = {
    tenantId,
    displayNameEn: tenantId === 'tnt_demo' ? 'Paritya Demo Tenant' : `Tenant ${tenantId}`,
    displayNameAr: tenantId === 'tnt_demo' ? 'مستأجر تجريبي' : `مستأجر ${tenantId}`,
  };
...
export function generateApiKeySecret(prefix = 'paritya_demo'): string {
