...
export async function startTestDb(): Promise<DynamoDBClient> {
  ...
  if (!process.env.TABLE_NAME && !process.env.DYNAMODB_CORE_TABLE_NAME) {
    process.env.TABLE_NAME = "paritya-core-test";
    process.env.DYNAMODB_CORE_TABLE_NAME = "paritya-core-test";
  }
  if (!process.env.BASE_TABLE_NAME && !process.env.DYNAMODB_BASE_TABLE_NAME) {
    process.env.BASE_TABLE_NAME = "paritya-base-test";
    process.env.DYNAMODB_BASE_TABLE_NAME = "paritya-base-test";
  }
  if (!process.env.ANALYTICS_TABLE_NAME && !process.env.DYNAMODB_ANALYTICS_TABLE_NAME) {
    process.env.ANALYTICS_TABLE_NAME = "paritya-analytics-test";
    process.env.DYNAMODB_ANALYTICS_TABLE_NAME = "paritya-analytics-test";
  }
