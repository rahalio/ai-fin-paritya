...
const rawEvidenceService = {
   ...
   * List evidence packs
   */
  async getEvidencePack(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/evidence-packs` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async createEvidencePack(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/evidence-packs`;
   ...
   * Get evidence pack
   */
  async getEvidencePack(evidencePackId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/evidence-packs/${evidencePackId}` + (params ? `?${new URLSearchParams(params).toString()}` : '');
