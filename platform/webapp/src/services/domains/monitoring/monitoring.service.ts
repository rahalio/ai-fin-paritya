...
const rawMonitoringService = {
   ...
   */
  async getMetric(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/monitoring/metrics` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async createMetric(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/monitoring/metrics`;
  ...
  async getBreach(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    ...
    const url = `/orgs/${orgId}//v1/monitoring/breaches` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async createBreach(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/monitoring/breaches`;
