...
const rawAppealsService = {
  /**
   * List appeal paths
   */
  async getAppeal(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/appeals` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   * Create appeal path
   */
  async createAppeal(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/appeals`;
   ...
   * Get appeal path
   */
  async getAppeal(appealPathId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/appeals/${appealPathId}` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   * Update appeal path
   */
  async updateAppeal(appealPathId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/appeals/${appealPathId}`;
...
export const appealsService = makeService(rawAppealsService, "appeals");
