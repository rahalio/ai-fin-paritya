...
const rawUsecasesService = {
   ...
   */
  async getUseCase(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/use-cases` + (params ? `?${new URLSearchParams(params).toString()}` : '');
  ...
  async createUseCase(data?: any, signal?: AbortSignal): Promise<any> {
    ...
    const url = `/orgs/${orgId}//v1/use-cases`;
  ...
  async getUseCase(useCaseId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    ...
    const url = `/orgs/${orgId}//v1/use-cases/${useCaseId}` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async updateUseCase(useCaseId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/use-cases/${useCaseId}`;
