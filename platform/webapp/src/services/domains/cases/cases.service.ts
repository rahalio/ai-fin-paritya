...
const rawCasesService = {
   ...
   * List cases
   */
  async getCase(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/cases` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   * Create case
   */
  async createCase(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/cases`;
   ...
   * Get case
   */
  async getCase(caseId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/cases/${caseId}` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   * Update case
   */
  async updateCase(caseId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/cases/${caseId}`;
