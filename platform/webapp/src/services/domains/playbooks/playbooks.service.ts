 ...
 * Playbooks Service
 *
 * API client for playbooks domain.
...
const rawPlaybooksService = {
  /**
   * List playbooks
   */
  async getPlaybook(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/playbooks` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   * Create playbook
   */
  async createPlaybook(data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/playbooks`;
   ...
   * Get playbook
   */
  async getPlaybook(playbookId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/playbooks/${playbookId}` + (params ? `?${new URLSearchParams(params).toString()}` : '');
...
export const playbooksService = makeService(rawPlaybooksService, "playbooks");
